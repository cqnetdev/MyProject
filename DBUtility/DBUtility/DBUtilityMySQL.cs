...
{
    /// <summary>
    /// MySQL数据访问类，可用于访问不同数据库
    /// Copyright (C) 2017-09-18 By Chenqing
    /// 需要引入DLL的名称为：MySql.Data.dll，版本是：6.9.9.0
    /// 禁止私自增加返回DataReader的方法！！！
    /// </summary>
   public class DBUtilityMySQL
    {
        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.    
        private string connectionString;
        public static string MySQLConnectionStringDB1 = ConfigurationManager.ConnectionStrings["ConnectionString1"].ToString();
        public static string MySQLConnectionStringDB2 = ConfigurationManager.ConnectionStrings["ConnectionString2"].ToString();
        public DBUtilityMySQL()
        {
            connectionString = MySQLConnectionStringDB1;
        }

        public DBUtilityMySQL(string DBConnectionString)
        {
